...
// Define a functional React component that takes in three props: friday, saturday, and total.
// Calculate the total lost sheep by summing the values in the friday and saturday arrays.
// Subtract the total lost sheep from the total property.
// Render the result in a paragraph tag.

const LostSheep = ({ friday, saturday, total }) => {
  const lost = total - (friday.reduce((a, c) => a + c, 0) + saturday.reduce((a, c) => a + c, 0));
  return <p>{lost}</p>;
};

//arrow function and spread
const lostSheep = (friday, saturday, total) => total - [...friday, ...saturday].reduce((a, c) => a + c, 0);
