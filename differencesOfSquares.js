...
function differenceOfSquares(n) {
  let sumOfSquares = ((n * (n + 1)) / 2) ** 2;
  let sumOfSquaresArr = [];
  let currentNumber = 1;

  while (currentNumber <= n) {
    sumOfSquaresArr.push(currentNumber ** 2);
    currentNumber++;
  }

  let squaresOfSums = sumOfSquaresArr.reduce((a, c) => a + c, 0);
  return sumOfSquares - squaresOfSums;
}

// fancy math solution
function differenceOfSquares(n) {
    let sum = (n * (n + 1)) / 2;
    let sumOfSquares = sum ** 2;
    let squaresOfSums = (n * (n + 1) * (2 * n + 1)) / 6;
