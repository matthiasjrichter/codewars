...
function eqSumPowdig(hMax, exp) {
  ...
  for (let i = 2; i <= hMax; i++){
    if (String(i).split("").reduce((a, c) => a + c ** exp, 0) === i) res.push(i)
  }
  return res;
}

// One line for fun
const eqSumPowdig = (hMax, exp) => Array.from({length: hMax - 1}, (_, i) => i + 2).filter(e => (e + "").split("").reduce((a, c) => a + c ** exp, 0) === e);