...
//     // arrow it
//     return archers.every( => );
//   }

function archersReady(archers) {
  if (archers.length === 0) return false;
  else return archers.every((element) => element >= 5);
}

//alternative 1
const archersReady = (archers) => archers.length === 0 ? false : archers.every((element) => element >= 5);

//alternative 2
function archersReady(archers) {
  return archers.length > 0 && archers.every((element) => element >= 5);
}