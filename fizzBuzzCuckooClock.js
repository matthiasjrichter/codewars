...
function fizzBuzzCuckooClock(time) {
  ...
  if (minutes === "00") return "Cuckoo ".repeat(hour > 12 ? hour - 12 : hour === "00" ? 12 : hour).trim();
  if (minutes === "30") return "Cuckoo";
  if (!(minutes % 3) && !(minutes % 5)) return "Fizz Buzz";
  if (!(minutes % 3)) return "Fizz";
  if (!(minutes % 5)) return "Buzz";
  return "tick";
}
