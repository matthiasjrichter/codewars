...
//recursion
function foldTo(distance, thickness = 0.0001, count = 0) {
  if (distance < 0) return null;
  if (thickness >= distance) return count;
  return foldTo(distance, thickness * 2, count + 1);
}

//recursion arrow function and ternary
const foldTo = (d, t = 0.0001, c = 0) => d < 0 ? null : t >= d ? c : foldTo(d, t * 2, c + 1);
