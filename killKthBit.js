...
//Index k positive integer (counted fromthe right)
//Output return integer

//convert n to binary
//convert to array
//replace k-th bit from the right with 0 then convert to integer and return

function killKthBit(n, k) {
    let nBin = n.toString(2).split("");
    if (nBin.length < k) {
      return parseInt(nBin.join(""), 2);
    } else {
      nBin.splice(nBin.length - k, 1, 0);
      return parseInt(nBin.join(""), 2);
    }
  }
