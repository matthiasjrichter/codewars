...
function freqMap(str) {
  const map = new Map();
  for (const n of str) map.set(+n, (map.get(+n) || 0) + 1);
  return map;
}
...
function paintLetterboxes(start, end) {
  ...
  const freq = freqMap(str), result = [];
  for (let i = 0; i < 10; i++) result.push(freq.get(i) || 0);
  return result;
}

// Okay I totally overenginnered this. Here is a simpler approach:
function paintLetterboxes(start, end) {
  const freq = Array(10).fill(0);
  for (let i = start; i <= end; i++) for (const digit of i.toString()) freq[digit]++;
  return freq;
}