...
function scoreTest(str, right, omit, wrong){
  ...
  for (const answer of str){
    ...
    else if (answer === 1) result += omit;
    else if (answer === 2) result -= wrong;
  }
  return result;
}

// Map object and reduce
function scoreTest(str, right, omit, wrong){
  const scores = new Map([[0, right], [1, omit], [2, -wrong]]);
  return str.reduce((a, c) => a + scores.get(c), 0);
}

// Array for lookup
function scoreTest(str, right, omit, wrong){
  const scores = [right, omit, -wrong];
  return str.reduce((a, c) => a + scores[c], 0);
}

// Ternary and reduce
const scoreTest = (str, right, omit, wrong) => str.reduce((a, c) => a + (c === 0 ? right : c === 1 ? omit : -wrong), 0);