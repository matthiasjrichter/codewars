...
function splitTheBill(x) {
  const total = Object.values(x).reduce((a, c) => a + c, 0);
  const average = total / Object.values(x).length;

  for (let key in x) {
    x[key] = Number((x[key] - average).toFixed(2));
  }
  return x;
}
...
const splitTheBill = (x) => {
  const values = Object.values(x);
  const average = values.reduce((a, c) => a + c, 0) / values.length;
  return Object.fromEntries(
    Object.entries(x).map(([k, v]) => [k, +(v - average).toFixed(2)])
  );
};
