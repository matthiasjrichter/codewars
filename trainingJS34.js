...
function cutCube(volume, n) {
  ...
  const smallCubeVolume = volume / n;
  const smallCubeSide = Math.round(Math.cbrt(smallCubeVolume));
  if (smallCubeSide ** 3 !== smallCubeVolume) return false;

  const originalCubeSide = Math.round(Math.cbrt(volume));
  if (originalCubeSide ** 3 !== volume) return false;
